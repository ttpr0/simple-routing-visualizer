...
<template>
    <div class="filetree">
        <FileTreeItem :path="path" :name="item.name" :type="item.type" :open="open" @click="onClick()" @contextmenu="onRightClick"></FileTreeItem>
        <div class="children" v-if="item.children !== undefined && open">
            <FileTree v-for="child in item.children" :key="child.name" :path="path + item.name + '/'" :item="child" :onclickHandler="onclickHandler"></FileTree>
        </div>
    </div>
</template>
