...
const pointsymbologybar = {
    ...
    setup(props) {
        ...
        function applyChanges() {
            const newStyle = new PointStyle(color.value, radius.value, point_type.value, points.value, inner_radius.value);
            const layer = map.getLayerByName(map_state.focuslayer);
            layer.setStyle(newStyle);
        }

        return { active, applyChanges, color, radius, point_type, points, inner_radius }
    },
    template: `
    <n-space vertical v-if="active">
        ...
        <p>type:</p>
        <n-select v-model:value="point_type" :options="[{label:'polygon',value:'polygon'},{label:'circle',value:'circle'},{label:'star',value:'star'}]" />
