...
export default {
    components: { Icon, NSpace, NInput, NTag, NScrollbar, ToolContainer, ToolTag, toolparam, TextInput, ProgressBar },
    props: [ ],
    setup(props) {
        ...
        onMounted(() => {
            if (toolbar.toolview.toolbox === toolbar.currtool.toolbox && toolbar.toolview.tool === toolbar.currtool.tool) {
                toolbar.toolview.params = toRaw(toolbar.currtool.params);
            } else {
                let tool = toolmanager.getTool(toolbar.toolview.toolbox, toolbar.toolview.tool);
        ...
        const out_info = computed(() => {
            const tool = toolmanager.getTool(toolbar.toolview.toolbox, toolbar.toolview.tool);
            if (tool !== undefined) {
        ...
        const runTool = async () => {
            ...
            const p = toRaw(toolbar.toolview.params);
            const out = await toolmanager.runTool(toolbar.toolview.toolbox, toolbar.toolview.tool, p)
            if (out == null)
                return;
        ...
        return { tool_params, param_info, tool_name , runTool, activateToolInfo, closeCurrTool, setParam}
    }
}
</script>

<template>
    <ToolContainer :toolbox="tool_name[1]" :tool="tool_name[0]" @close="closeCurrTool()" @run="runTool()" @info="activateToolInfo()">
