...
export default {
    components: { Icon, NSpace, NInput, NTag, NScrollbar, ToolContainer, ToolTag, TextInput },
    props: [ ],
    setup(props) {
        ...
        const search_list = computed(() => {
            ...
            for (const toolbox in toolbar.tools) {
                ...
                for (const item of tools.filter(element => element.toLowerCase().includes(search.value))) {
        ...
        return { search, setSearch, tool_dict, search_list, setCurrTool, opened, toggleDetails}
    }
}
</script>
...
<template>
    <div style="height: 100%;">
        <TextInput :modelValue="search" @update:modelValue="setSearch" placeholder="Filter Tools" />
        <div style="height: calc(100% - 34px); padding-top: 20px;">
            <n-scrollbar>
                <div v-if="search!==''">
                    <div class="taglist" v-for="[item, toolbox] in search_list" :key="item">
                        <ToolTag :tool="item" :toolbox="toolbox" @click="setCurrTool(toolbox, item)" />
                    </div>
                </div>
                <div v-else>
                    <details v-for="(tools, toolbox) in tool_dict" :key="toolbox" :open="opened[toolbox]" @toggle="e => toggleDetails(toolbox, e)">
