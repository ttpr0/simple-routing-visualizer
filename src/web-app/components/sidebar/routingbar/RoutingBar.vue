...
import {
  getRouting,
  getRoutingDrawContext,
  getRoutingStep,
} from "/util/routing/api";
...
export default {
  ...
  setup(props) {
    ...
    watch(layers, () => {
      const l = map_state.layers.find(
        (layer) => layer.name === "routing_points"
      );
      if (l === undefined) {
        CONFIG["app"]["sidebar"] = CONFIG["app"]["sidebar"].filter(
          (elem) => elem.comp !== "RoutingBar"
        );
    ...
    const runRouting = async () => {
      ...
      try {
        if (params["draw"]) {
          let context = await getRoutingDrawContext(
            startpoint,
            endpoint,
            routing_type
          );
          ...
          var finished = false;
          var geojson = null;
          let routinglayer = new VectorImageLayer(
            [],
            "LineString",
            "routing_layer"
          );
          ...
          routinglayer = new VectorImageLayer(
            geojson["features"],
            "LineString",
            "routing_layer"
          );
    ...
    <n-space vertical>
      ...
      <n-select
        ...
        :options="[
          ...
          { label: 'AStar', value: 'A*' },
          { label: 'Bidirectional Dijkstra', value: 'Bidirect-Dijkstra' },
          { label: 'Bidirectional AStar', value: 'Bidirect-A*' },
        ]"
      />
